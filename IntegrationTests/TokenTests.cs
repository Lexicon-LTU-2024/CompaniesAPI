...
public class TokenTests : IClassFixture<WebApplicationFactory<Program>>
{
  ...
  //  private IServiceManager serviceManager;
    ...
    public TokenTests(WebApplicationFactory<Program> applicationFactory)
    {
        ...
        db = scope.ServiceProvider.GetRequiredService<DBContext>();
    }
    ...
    public async Task GetTokenAndTryToGetResponse()
    {
        ...
        var res = await httpClient.PostAsJsonAsync("auth/login", new UserForAuthDto { UserName = "kalle", Password = "bytmig" });
        ...
        Assert.Equal(exspectedCount, dtos?.Count());
